...
class Planner:
    ...
    def build(
        ...
        main_meta = self.manifest.load_json(dest / file_name)

        plan = Plan()

        keys_in_template, removed_in_template = self.manifest.collect_keys(template_meta, main_meta)

        # Updates / Adds
        for key in keys_in_template:
            t_info = template_meta[key]
            m_info = main_meta.get(key)
            value = (key, t_info['template_path'])
            ...
            # Both sides have metadata
            if t_info['last_commit'] == m_info['last_commit']:
                plan.skip.append(value)
                continue

            current_hash = self.hasher.md5_file(project_path)
            if current_hash is None or current_hash == m_info['md5']:
                plan.auto_update.append(value)
            else:
                plan.prompt_user.append(value)
        ...
        for key in removed_in_template:
            m_info = main_meta[key]
            value = (key, m_info['template_path'])
...
class TCVHelper:
    ...
    def _run(self, branch: str, template_name: str, template_type: str, dest=Path(), force=False):
    ...
    def init(self, branch: str, template_name: str, template_type: str, dest=Path(), force=True):
        """Init template files in `dest` directory."""
        self._run(branch, template_name, template_type, dest, force)

    def update(self, branch: str, template_name: str, template_type: str, dest=Path(), force=False):
